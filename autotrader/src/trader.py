...
class Trader:
    ...
    def setup_trade_session(self) -> TradeSession:
        ...
        logger.info(f"Setting up session {self.session_metadata}")
    ...
    def trade(self, calc_cycle: int, cur_time: float) -> None:
        ...
        for strat in strats_to_compute:
            ...
            actions: List[OrderAction] = strat.compute(exchange=exchange, inventory=inventory, price_data=price_data)
            if not actions:
                logger.info("No order actions generated")
                continue
